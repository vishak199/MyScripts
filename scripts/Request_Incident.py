...
def sync_data(csv_file, table):
    ...
    if len(rows) > 0:
        ...
        if len(new_and_update_rows) > 0:
            ...
            #elif table in RELATIONSHIP_TABLES or table in TRANSACTION_TABLES: ---Arbaz
                #if not insert_records(table, columns, temp_csv_file, filename): ---Arbaz
                    #return ---Arbaz
                #SUCCESS_LIST.append(filename) ---Arbaz
