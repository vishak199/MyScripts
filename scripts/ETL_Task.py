...
def create_temp_csv(rows, cols, filename):
    ...
    file_    = os.path.join('/tmp/ETLTask', filename)
...
def sync_data(csv_file, table):
    ...
    if len(rows) > 0:
	#logging.info(f'executing length of rows condition')
	#Vinai Comment
				
        # get records count before sync
        records_count_before_sync = get_records_count(table)
	#logging.info(f'executing records count before sync') #Vinai Comment
	...
	#logging.info(f'new and update rows, delete rows') 	#Vinai Comment
	...
	#logging.info(f'executing new and update rows condition') 	#Vinai Comment
			
            # verify the columns
            new_and_update_rows = verify_columns(new_and_update_rows, table, filename)
			
	#logging.info(f'new and update rows condition-verify the columns') 	#Vinai Comment

            columns = list(new_and_update_rows[0].keys())
            columns_lower = [col.lower() for col in columns]
	#logging.info(f'executing columns, columns_lower') 	#Vinai Comment

            # create temp csv file
            temp_csv_file = create_temp_csv(new_and_update_rows, columns, filename)
	#logging.info(f'created temp csv file') 	#Vinai Comment

            # check table type
            if table in MASTER_TABLES:
                # get transaction table name
                trans_table = f'{table}__transaction'
		#logging.info(f'executed if cond to check table in Master tables') #Vinai Comment

                # delete if any existing records from transaction table
                if not delete_trans_records(trans_table, filename):
                    return
			#logging.info(f'executed if not delte trans records') 	#Vinai Comment

                # insert record to transaction table
                if not insert_records(trans_table, columns_lower, temp_csv_file, filename):
                    return
				#logging.info(f'executed if not insert records') 	#Vinai Comment
				
                # merge tables
                if not merge_data(table, trans_table, filename, columns_lower):
                    return 
				#logging.info(f'executed if not merge data') 	#Vinai Comment

                SUCCESS_LIST.append(filename) 

            elif table in RELATIONSHIP_TABLES or table in TRANSACTION_TABLES: # Arbaz comment
                if not insert_records(table, columns, temp_csv_file, filename): # Arbaz comment
                    return #Arbaz comment
                SUCCESS_LIST.append(filename) # Arbaz comment
		...
		#logging.info(f'executed else condition') 	#Vinai Comment
        SUCCESS_LIST.append(filename)
...
def main():
    ...
    #all_tables = MASTER_TABLES + TRANSACTION_TABLES + RELATIONSHIP_TABLES ---Arbaz
    ...
    for file_ in files:        
        ...
        #table = file_[:_last_index].lower() ---Arbaz
        table =str(newtable) + file_[:_last_index]
        if 'comment' in table:
            table = 'comments'        
