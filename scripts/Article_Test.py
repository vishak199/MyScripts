...
KEY_TO_UPDATE = {
    "testarticlebaseonrequest": ["firstentity_request", "secondentity_article"],
    "testarticle": "id",
    "testdevice": "id",
    "testservicecomponent": "id",
    "testsystemelement": "id",
    "testservicecomponentcontainsdevice": ["firstentity_request", "secondentity_article"],
    "testsystemelementcontainsdevice": ["firstentity_request", "secondentity_article"], 
}
...
RELATIONSHIP_TABLES = [
    'testarticlebaseonrequest',
    'testservicecomponentcontainsdevice',
    'testsystemelementcontainsdevice',
]

TRANSACTION_TABLES = [
    'testdevice_cpu',
    'testdevice_diskdevice',
    'testdevice_filesystem',
    'testdevice_ipaddress',
    'testdevice_networkcard',
    'testdevice_runningsoftware',
    'comments',
]
...
def create_temp_csv(rows, cols, filename):
    ...
    file_    = os.path.join('/tmp/Article', filename)
...
def merge_data(table, trans_table, filename, columns):
    ...
    if type(unique_id) == str:
       ...
       solution = execute_sql_command(matching_cond, filename)
       ...
       if solution:
          logging.info("coming to update......")
          cmd = f"UPDATE {ARGS.schema}.{table} TGT SET {cols_to_update} FROM {ARGS.schema}.{trans_table} SRC WHERE {matching_cond}"
          result = execute_sql_command(cmd, filename)
       elif not solution:
          logging.info("coming to insert.......")
          cmd = f"INSERT INTO {ARGS.schema}.{table} ({cols_to_insert}) SELECT {insert_data} FROM {ARGS.schema}.{trans_table} SRC"
          result = execute_sql_command(cmd, filename)
       return result
...
def update_data(table, trans_table, filename, columns):
    """
    Executes merge command
    """
    logging.info(f'merging table data: {trans_table} -> {table}')
...
def insertion_data(table, trans_table, filename, columns):
    """
    Executes merge command
    """
    logging.info(f'merging table data: {trans_table} -> {table}')
    ...
    # construct insertion command
    insert_data = 'SRC.' + ',SRC.'.join(columns)
    logging.info(f'insert_data: {insert_data}')

    # construct merge command
    cmd = f"INSERT INTO {ARGS.schema}.{table} ({cols_to_insert}) SELECT {insert_data} FROM {ARGS.schema}.{trans_table} SRC WHERE SRC.{unique_id} NOT IN (SELECT {unique_id} FROM {ARGS.schema}.{table})"
    return execute_sql_command(cmd, filename)
    ...
    if len(rows) > 0:
        ...
        if len(new_and_update_rows) > 0:
            ...
            if table in MASTER_TABLES:
                ...
                # merge tables
                if not update_data(table, trans_table, filename, columns_lower):
                    return
                    
                if not insertion_data(table, trans_table, filename, columns_lower):
                    return
